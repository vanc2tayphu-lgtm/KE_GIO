...
4. Trong Apps Script, chọn hàm `setupAuthorization`, bấm `Run`, rồi cấp quyền gửi mail khi Google hỏi.
5. Bấm `Deploy` -> `New deployment` -> chọn loại `Web app`.
6. `Execute as`: tài khoản của bạn. `Who has access`: Anyone with the link.
7. Copy Web App URL và dán vào ô `Apps Script Web App URL` trong app.

Apps Script sẽ tạo thêm sheet `Danh sach giao vien` gồm các cột `Ma GV`, `Ho ten GV`, `Mon`, `Email`, `Ma bao mat`. Admin điền email cho từng giáo viên và có thể chỉnh môn/mã bảo mật tại đây. Khi giáo viên bấm `Lưu bảng kê`, app yêu cầu nhập `Ma bao mat` 6 số; mã đúng mới được ghi vào sheet `Tong hop ke gio`. Nếu giáo viên quên mã, dùng nút `Quên mã` để gửi mã mới về email đã đăng ký.

Sheet `Tong hop ke gio` có mỗi giáo viên một dòng. Bảng có cột `Số tt`, `Họ tên GV`, `Môn`; từ tháng 9 đến tháng 5, mỗi tháng được merge 3 cột: `Tổng`, `Thừa`, `Thiếu`.
